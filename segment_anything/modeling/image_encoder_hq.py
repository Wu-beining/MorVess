...
class ImageEncoderViT_hq(nn.Module):
    def __init__(
        self,
        img_size: int = 1024,
        patch_size: int = 16,
        ...
        self.proj_early = nn.Conv2d(embed_dim,out_chans,kernel_size=1,bias=False)
        self.ln_early = LayerNorm2d(out_chans)
    ...
    def forward_features(self,x:torch.Tensor):
        ...
        return F_last, F_early, F_last
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        img_embed,_,_ = self.forward_features(x)
        return img_embed
