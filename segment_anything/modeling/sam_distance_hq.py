...
class Sam_distance_hq(nn.Module):
    ...
    def forward_train(self, batched_input, multimask_output, image_size):
        ...
        try:
            image_embeddings,enc_early_64,enc_last_64 = self.image_encoder(input_images, d_size)
        except TypeError:
            image_embeddings, enc_early_64, enc_last_64 = self.image_encoder.forward_features(input_images)
...
class Sam_multi_hq(nn.Module):
    ...
    def forward_train(self, batched_input, multimask_output, image_size):
        ...
        # image_embeddings = self.image_encoder(input_images, d_size)
        try:
            image_embeddings, enc_early_64, enc_last_64 = self.image_encoder.forward_features(input_images,d_size)
            # image_embeddings, enc_early_64, enc_last_64 = self.image_encoder(input_images, d_size)
        except TypeError:
            image_embeddings, enc_early_64, enc_last_64 = self.image_encoder.forward_features(input_images)
