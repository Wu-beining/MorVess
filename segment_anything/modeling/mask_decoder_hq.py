...
from .hq_refiner import HQRefiner
...
class MaskDecoder_distance(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.hq_token = nn.Embedding(1, transformer_dim)
...
class MaskDecoder_multi_hq(nn.Module):
    def __init__(
        self,
        *,
        transformer_dim: int,
        ...
        self.hq_proj_dec = nn.Conv2d(transformer_dim // 32, transformer_dim, kernel_size=1, bias=False)

        self.hq_up_early = nn.Sequential(
        ...
        # self.hq_refiner = HQRefiner(c=transformer_dim, use_geom=True, film=False, geom_ih=3)
        self.hq_refiner = HQRefiner(c=transformer_dim, use_geom=True, film=False)
        self.hq_gate = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(
        ...
        dense_prompt_embeddings: torch.Tensor,
        multimask_output: bool,
        enc_early_64: torch.Tensor = None,
        enc_last_64: torch.Tensor = None, # b c 64 64 
    ...
    def predict_masks(
        ...
        # tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0)
        # iou + k和mask + hq
        tokens = torch.cat([self.iou_token.weight,self.mask_tokens.weight,self.hq_token.weight], dim=0)
        point_embedding = tokens.unsqueeze(0).expand(B, -1, -1)
        ...
        hq_token_out = point_tokens[:, 1 + self.num_mask_tokens, :]
        ...
        F_dec_256 = self.hq_proj_dec(upscaled_embedding)  # (B, C, H, W)
        if enc_early_64 is not None:
            F_early_256 = self.hq_up_early(enc_early_64)
        else:
            F_early_256 = torch.zeros_like(F_dec_256)
        if enc_last_64 is not None:
            F_last_256 = self.hq_up_last(enc_last_64)
        else:
            F_last_256 = torch.zeros_like(F_dec_256)
        ...
        alpha = torch.sigmoid(self.hq_gate)
